    ...
    SubShader
    {
        ...
        Pass
        {
            ...
            float2 rayBoxDst(float3 boundsMin, float3 boundsMax, float3 rayOrigin, float3 invRaydir) {
                ...
                float3 t0 = (boundsMin - rayOrigin) * invRaydir; // for the point on boundsMin - 0
                float3 t1 = (boundsMax - rayOrigin) * invRaydir; // for the point on boundsMax - 1
                ...
                // get intersection distance - dstA for the first intersection with box, dstB for the second
                float dstA = max(max(tmin.x, tmin.y), tmin.z);
                float dstB = min(tmax.x, min(tmax.y, tmax.z));

                // ray intersected the box from the outside if 0 <= dstA <= dstB
                float dstToBox = max(0, dstA);

                // ray intersected the box from the inside if dstA < 0 < dstB (dstA < 0 < dstB)
            ...
            // implementing the phase function "blend" is a parameter in [-1,1]
            float phase(float a) {
                float blend = .5;
            ...
            // remaps a value from one range (low) to new range (high)
            ...
            float lightmarch(float3 p) {
                // normalized vector to light position
            ...
            float4 frag (v2f i) : SV_Target
            {
                ...
                // implementing the phase function, cosAngle is the cosine of the angle between two vectors
                ...
                while (dstTravelled < dstLimit) {
                    ...
                    float density = sampleDensity(rayPos); // get the density for the current part of the ray
                    ...
                    if (density > 0) {
                        ...
                        // Early exit
                        //performance measures, when density is zero it usually means we're out of the clouds
